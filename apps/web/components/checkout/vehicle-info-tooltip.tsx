  ...
  return (
    ...
    >
      <div
        ...
        data-phase={phase}
        className="bg-[#0D0D0D] border-x border-t md:border border-[rgba(255,255,255,0.12)] rounded-t-2xl md:rounded-none shadow-2xl w-full md:max-w-2xl max-h-[90vh] md:max-h-[85vh] overflow-y-auto flex flex-col max-md:translate-y-full max-md:data-[phase=entered]:translate-y-0 max-md:data-[phase=exiting]:translate-y-full max-md:transition-transform max-md:duration-300 max-md:ease-out md:animate-in md:fade-in md:zoom-in-95 md:duration-200 md:data-[phase=exiting]:opacity-0 md:transition-opacity md:duration-200"
        onTransitionEnd={handleTransitionEnd}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="sticky top-0 bg-[#0D0D0D] z-10 px-4 md:px-8 pt-4 md:pt-8 pb-2 md:pb-0 md:mb-6">
          <h2
            className="text-2xl md:text-4xl font-semibold text-center text-[#F7F4EF] pr-12 md:pr-0"
            style={SERIF_FONT}
          >
            {vehicleName}
          </h2>
          <button
            type="button"
            onClick={handleClose}
            className="absolute right-4 top-4 md:right-6 md:top-6 flex items-center justify-center w-9 h-9 md:w-10 md:h-10 rounded-full bg-[rgba(255,255,255,0.06)] hover:bg-[rgba(255,255,255,0.12)] text-[#F7F4EF] transition-colors"
            ...
            <div className="space-y-2 md:space-y-3">
              {included.map((item, i) => (
                <div key={i} className="flex items-start gap-2 md:gap-3">
                  ...
                  />
                  <div>
                    <p className="text-[#F7F4EF] text-sm md:text-base font-semibold leading-snug">
                      {item.text}
                    </p>
                    {item.sub && (
                      <p className="text-[#999] text-xs md:text-sm">{item.sub}</p>
                    )}
                  </div>
                </div>
              ))}
