          ...
          <div className="relative bg-[#0E4659] rounded-[16px] h-[200px] xl:h-[240px] [clip-path:inset(-500px_0_0_0_round_16px)]">
            ...
            <div className="absolute left-0 top-0 bottom-0 flex flex-col justify-center gap-6 md:gap-8 p-6 xl:p-8 z-10 max-w-[55%]" suppressHydrationWarning>
              <h2 className="text-[24px] md:text-[36px] font-bold leading-[1.1]">
                <span className="font-bold text-white">{t("titleLine1")}</span>
                <br />
              ...
              <form onSubmit={handleSubmit} className="flex gap-4 w-full max-w-[620px]" suppressHydrationWarning>
                <div className="relative flex-1 min-w-[400px]">
                  <Mail className="absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-[#808080]" />
                  <input
                    type="email"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder={t("placeholder")}
                    className="w-full h-[64px] pl-14 pr-5 bg-white rounded-[12px] text-[#222] placeholder:text-[#808080] text-[16px] focus:outline-none focus:ring-2 focus:ring-[#27C7FF] transition-all"
                    autoComplete="off"
                ...
                <button
                  type="submit"
                  className="h-[64px] px-8 bg-[#27C7FF] rounded-[12px] text-white font-bold text-[16px] uppercase tracking-wide flex items-center gap-2 hover:brightness-110 transition-all shrink-0"
                >
                  {t("button")}
              ...
              <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 w-full" suppressHydrationWarning>
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder={t("placeholder")}
                    className="w-full h-[48px] pl-12 pr-4 bg-white rounded-[8px] text-[#222] placeholder:text-[#808080] text-[14px] focus:outline-none focus:ring-2 focus:ring-[#27C7FF] transition-all"
                    autoComplete="off"
                ...
                <button
                  type="submit"
                  className="h-[48px] px-5 bg-[#27C7FF] rounded-[8px] text-white font-bold text-[14px] uppercase tracking-wide flex items-center justify-center gap-2 hover:brightness-110 transition-all shrink-0"
                >
                  {t("button")}
