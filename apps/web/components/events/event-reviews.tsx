...
import { EventReviewForm } from "./event-review-form"

interface EventReviewsProps {
  eventId: Id<"events">
  rating: number
  reviewCount: number
  initialReviews?: LuxmotionReview[]
}
...
export function EventReviews({
  ...
  const liveReviews = useQuery(api.tourReviews.listApprovedByEvent, { eventId })

  const reviews = useMemo<LuxmotionReview[]>(() => {
    if (liveReviews) {
  ...
  return (
    <div className="w-full flex flex-col gap-[20px]">
          ...
          <div className="hidden md:flex flex-col gap-[3px] pt-[12px]">
            {reviews.map((review, index) => (
              <LuxmotionReviewCard
                key={index}
                review={review}
                translationsNamespace="eventDetails"
              />
            ))}
          </div>
      ...
      <div className="w-full pt-[12px]">
        <EventReviewForm eventId={eventId} />
      </div>
    </div>
  )
}
