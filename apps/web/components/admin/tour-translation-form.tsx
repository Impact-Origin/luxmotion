...
import { Loader2, Globe, Check, Plus, Trash2 } from "lucide-react"
import { Separator } from "@workspace/ui/components/separator"
import { cn } from "@workspace/ui/lib/utils"

interface TourTranslationFormProps {
  isOpen: boolean
  onClose: () => void
  tourId: string
  tourTitle: string
...
export function TourTranslationForm({
  ...
  const [description, setDescription] = React.useState<unknown>(null)
  ...
  const translation = useQuery(
    api.tours.getTranslation,
    selectedLocale && selectedLocale !== originalLanguage
      ? { tourId: tourId as Id<"tours">, locale: selectedLocale }
      : "skip"
  )

  const upsertTranslation = useMutation(api.tours.upsertTranslation)
  const removeTranslation = useMutation(api.tours.removeTranslation)
  ...
  React.useEffect(() => {
    if (translation) {
      ...
      setCancellationPolicy(translation.cancellationPolicy || "")
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] p-0 flex flex-col overflow-hidden">
        <DialogHeader className="p-6 border-b shrink-0">
          ...
          <DialogDescription>
            Translate "{tourTitle}" into different languages
          </DialogDescription>
        </DialogHeader>

        <div className="flex-1 flex overflow-hidden">
          <div className="w-[240px] border-r bg-muted p-4 overflow-y-auto shrink-0">
            <div className="space-y-1">
              <div className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                Original
              </div>
              <div className="px-3 py-2 rounded-lg bg-accent text-foreground font-medium flex items-center gap-2">
            ...
            {selectedLocale ? (
              ...
              <div className="flex-1 flex items-center justify-center text-muted-foreground">
                <div className="text-center">
                  <Globe className="h-12 w-12 mx-auto mb-4 text-muted-foreground/60" />
